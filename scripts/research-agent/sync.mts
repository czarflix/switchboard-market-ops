...
async function patchAgentConfig(
  runtime: ReturnType<typeof readResearchAgentRuntimeEnv>,
  request: AgentUpdateRequestWithGuardrails,
) {
  const url = new URL(`v1/convai/agents/${runtime.agentId}`, "https://api.elevenlabs.io/");
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Agent sync failed with ${response.status} ${response.statusText}${body ? `: ${body}` : ""}`,
    );
  }
...
async function resolveSaveResearchBriefTool(client: ReturnType<typeof createElevenLabsClient>, baseUrl: string, intakeSecret: string) {
  ...
  const desiredToolRequest = buildResearchBriefToolConfig(baseUrl, intakeSecret) as unknown as ToolCreateRequest;

  if (exactMatches.length > 1) {
    throw new Error(
      `Expected exactly one ${RESEARCH_AGENT_TOOL_NAME} tool, found ${exactMatches.length}. Remove duplicate tools before syncing.`,
    );
  }
  ...
  const liveTool =
    ...
    (() => {
      if (matches.length !== 1) {
        throw new Error(
          `Expected exactly one compatible research handoff tool to migrate, found ${matches.length}. Remove duplicates before syncing.`,
        );
      }
  ...
  const updated = await client.conversationalAi.tools.update(liveTool.id, desiredToolRequest);
...
async function resolveResearchPostCallWebhook(
  ...
  if (exactNameMatches.length > 1 && !exactUrlMatch) {
    throw new Error(
      `Expected at most one ${RESEARCH_AGENT_POST_CALL_WEBHOOK_NAME} webhook, found ${exactNameMatches.length}. Remove duplicates before syncing.`,
    );
  }

  if (exactUrlMatch) {
    if (dryRun) {
      if (exactUrlMatch.isDisabled || exactUrlMatch.name !== RESEARCH_AGENT_POST_CALL_WEBHOOK_NAME) {
...
async function main() {
  ...
  if (runtime.baseUrl.startsWith("http://localhost") && !allowLocalhost) {
    throw new Error(
      "Set RESEARCH_AGENT_WEBHOOK_BASE_URL, APP_BASE_URL, NEXT_PUBLIC_APP_URL, NEXT_PUBLIC_SITE_URL, or VERCEL_URL before syncing. Pass --allow-localhost only for local dry runs.",
    );
  }

  console.log(heading("Research agent sync"));
  console.log(`Agent: ${runtime.agentId}`);
  console.log(`Branch: ${runtime.branchId ?? "(main)"}`);
  console.log(`Base URL: ${runtime.baseUrl}`);
  console.log(`Dry run: ${dryRun ? "yes" : "no"}`);

  const tool = await resolveSaveResearchBriefTool(client, runtime.baseUrl, runtime.intakeSecret);
  ...
  const updateRequest = buildResearchAgentUpdateRequest(
    tool.toolId,
    postCallWebhookId,
  ) as unknown as AgentUpdateRequestWithGuardrails;
  const requestWithBranch: AgentUpdateRequestWithGuardrails = runtime.branchId
    ? ({ ...updateRequest, branchId: runtime.branchId } as AgentUpdateRequestWithGuardrails)
    : updateRequest;

  if (dryRun) {
    const liveAgent = await client.conversationalAi.agents.get(
      runtime.agentId,
      runtime.branchId ? { branchId: runtime.branchId } : undefined,
    );
    const agentDiffs = diffValues(
      pickAgentSnapshot({
      ...
      pickAgentSnapshot({
        name: liveAgent.name,
        conversationConfig: liveAgent.conversationConfig,
        platformSettings: liveAgent.platformSettings,
        tags: liveAgent.tags,
        branchId: liveAgent.branchId ?? undefined,
      }),
    );

    if (agentDiffs.length === 0) {
      console.log("Agent already matches desired config.");
    } else {
      console.log("Agent diff:");
      for (const diff of agentDiffs) {
        console.log(`- ${diff}`);
      }
    }

    console.log("\nDesired agent request:");
    console.log(stableJson(requestWithBranch));
    return;
  }

  const updatedAgent = await patchAgentConfig(runtime, requestWithBranch);

  console.log(`Updated agent: ${updatedAgent.agentId}`);
  console.log(`Current branch: ${updatedAgent.branchId ?? "(main)"}`);
